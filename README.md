...
- **"Libro de transferencias" (control diario, septiembre 2026 en adelante)** — **fuente vigente de transferencias**: `src/lib/import/libro-transferencias.ts` + sync diario `scripts/sync-libro-transferencias.mts` (workflow `sync-libro-transferencias.yml`, secret `LIBRO_TRANSFERENCIAS_URL`). Una fila por transferencia: el cliente entrega pesos (`MONTO RECIBIDO`) y el beneficiario recibe dólares (`MONTO USD`), ruta MX→EEUU. El archivo se lleva a mano y algunas filas traen una columna corrida, así que **no se recalcula la utilidad**: se toma la columna `TOTAL CASA` tal cual, anclándose en la celda `CUENTA` (`P19`, `P22`…) para ubicarla. `spread = MONTO RECIBIDO − MONTO USD × TC COMPRA`; `costos = spread − TOTAL CASA` (comisión al comisionista + al banco); `net_profit = TOTAL CASA`.
- **"ESTADO DE CUENTA CLIENTES" (una hoja por cliente)** — *fuente histórica, sync automático desactivado el 2026-09-02*: se lee sin asistente de columnas (`src/lib/import/estado-cuenta.ts`); el workflow `sync-estado-cuenta.yml` quedó solo para disparo manual. **Reglamento de utilidad**: la utilidad de cada pago = **comisión** (columna `COM $`, o `COM %` × TOTAL) **+ spread cambiario**, y el spread — `USD × (TC venta − TC compra)` — solo se suma cuando la hoja trae TC de compra **y** de venta con datos completos. Si falta el TC de compra se iguala al de venta (spread 0) y no se suma nada por diferencia de tipo de cambio; la columna `DIFERENCIA` de la hoja no se usa como fuente porque a menudo viene sin el TC de compra restado.
