...
- **Márgenes en transferencias importadas**: las filas importadas entran con spread 0 (la tabla del equipo no lleva tipo de cambio de compra ni comisión). Si más adelante se agregan esas columnas a la fuente, `src/lib/import/transfer-import.ts` es el único lugar a tocar para calcular la utilidad real.
